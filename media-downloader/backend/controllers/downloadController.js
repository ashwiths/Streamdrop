...
export const getMediaInfo = async (req, res) => {
  try {
    ...
    if (isYouTubeUrl(url)) {
      ...
      try {
        console.log(`[getMediaInfo] Local environment: Routing YouTube URL through yt-dlp...`);
...
export const downloadFile = async (req, res) => {
  try {
    const { url, format, ext, type, title, quality } = req.body;
    ...
    if (isYouTubeUrl(url)) {
      ...
      const tempDir = os.tmpdir();
        ...
        console.log(`[downloadFile] Local environment: Starting yt-dlp temp file download → ${tempFilePathTemplate}`);
        
        try {
          await downloadToTempFile(url, downloadType, format, tempFilePathTemplate, quality);
